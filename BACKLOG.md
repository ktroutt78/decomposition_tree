...
| EBL-010 | **S** | **Tooltip: fix title truncation and remove misleading drill text** |
| EBL-011 | **M** | **Smart zoom on drill — focus on drilled node and its new children** | When a user drills into a node, instead of fitting the entire tree into view (which pushes the root off-screen and makes nodes tiny), zoom to frame only the drilled node (parent) and its newly revealed children. This keeps the context of where the drill originated while maximizing readability of the new level. The existing fit-to-view button (⊡) remains available for users who want to see the full tree at any time. Requires computing the bounding box of just the active parent + its children after each drill event, then animating the D3 zoom to that frame. Should trigger on drill-down only — not on collapse, filter changes, or re-expansion of a previously collapsed node. Add a toggle in Settings (on by default) so users can revert to the original fit-all behavior if preferred. | Two issues: (1) Long node labels (e.g. "Business and Financial Operations") are cut off in the tooltip header — fix by allowing the title to wrap to multiple lines or by making the tooltip width dynamic rather than fixed. (2) The "Click to drill down" instructional text at the bottom of the tooltip is misleading — drilling only happens via the + button, not by clicking the node. Remove or replace this text with accurate guidance (e.g. "Click + to expand"). | Add a button inside the tooltip to open a detail drawer showing all rows that make up the selected node — displaying dimension attributes from the Marks card. Requires a new modal/table component, row pagination for large datasets, and formatting logic. |
